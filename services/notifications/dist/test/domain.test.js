    ...
    return {
        ...
        markAllRead: async (rid) => {
            ...
            for (const [id, rec] of notifications) {
                if (rec.recipientId === rid && !rec.read) {
                    notifications.set(id, { ...rec, read: true });
                    count++;
                }
            }
            return count;
...
test('renderTemplate: unknown type returns type as title', () => {
    const { title } = renderTemplate('nonexistent_type', {}, 'en');
...
test('inAppProvider always sends', async () => {
    const result = await inAppProvider.send({ id: 'n1', title: 'Test', body: 'Body' });
    assert.equal(result.status, 'sent');
    assert.equal(result.channel, 'in_app');
});
test('emailProvider suppresses if no address', async () => {
    const result = await emailProvider.send({ id: 'n2', title: 'Test', body: 'Body' });
    assert.equal(result.status, 'suppressed');
});
test('emailProvider sends when address provided', async () => {
    const result = await emailProvider.send({ id: 'n3', title: 'Test', body: 'Body' }, 'user@example.com');
    assert.equal(result.status, 'sent');
});
test('smsProvider suppresses if no number', async () => {
    const result = await smsProvider.send({ id: 'n4', title: 'Test', body: 'Body' });
    assert.equal(result.status, 'suppressed');
});
/* ─── sendNotification ────────────────────────────────────────── */
test('sendNotification: creates in_app delivery with default preference', async () => {
    const repo = makeRepo();
    const rec = await sendNotification({ recipientId: 'emp1', entityId: 'ent1', type: 'leave_approved',
        vars: { leaveType: 'Annual', startDate: '2026-03-15', endDate: '2026-03-22', days: '6' } }, repo, ALL_PROVIDERS);
...
test('sendNotification: renders in recipient locale (ar)', async () => {
    ...
    const rec = await sendNotification({ recipientId: 'emp_ar', entityId: 'ent1', type: 'leave_approved',
        vars: { leaveType: 'سنوية', startDate: '2026-03-15', endDate: '2026-03-22', days: '6' } }, repo, ALL_PROVIDERS);
...
test('sendNotification: fans out to opted-in channels', async () => {
    ...
    const rec = await sendNotification({ recipientId: 'emp2', entityId: 'ent1', type: 'letter_issued',
        vars: { letterType: 'Salary Certificate', documentId: 'doc_x' },
        priority: 'normal' }, repo, ALL_PROVIDERS, { email: 'emp2@example.com' });
...
test('sendNotification: suppresses non-in_app during quiet hours for normal priority', async () => {
    ...
    const rec = await sendNotification({ recipientId: 'emp3', entityId: 'ent1', type: 'leave_submitted',
        vars: { leaveType: 'Annual', startDate: '2026-06-01', endDate: '2026-06-07' },
        priority: 'normal' }, repo, ALL_PROVIDERS);
...
test('sendNotification: urgent priority ignores quiet hours', async () => {
    ...
    const rec = await sendNotification({ recipientId: 'emp4', entityId: 'ent1', type: 'approval_required',
        vars: { title: 'Leave', requesterName: 'X', slaDueAt: '17:00' },
        priority: 'urgent' }, repo, ALL_PROVIDERS, { sms: '+966501234567' });
    const channels = rec.deliveries.map((d) => d.channel);
    assert.ok(channels.includes('sms'), 'urgent should bypass quiet hours');
});
test('sendNotification: rejects missing recipientId', async () => {
    const repo = makeRepo();
    await assert.rejects(() => sendNotification({ recipientId: '', entityId: 'ent1', type: 'leave_approved', vars: {} }, repo, ALL_PROVIDERS), (e) => e.code === 'VALIDATION');
});
test('sendNotification: stores sourceEventType', async () => {
    const repo = makeRepo();
    const rec = await sendNotification({ recipientId: 'emp5', entityId: 'ent1', type: 'approval_required',
        vars: { title: 'Leave', requesterName: 'Y', slaDueAt: '13:00' },
        sourceEventType: 'StepActivated', sourceEventId: 'evt_abc' }, repo, ALL_PROVIDERS);
...
test('markRead: marks single notification as read', async () => {
    const repo = makeRepo();
    const sent = await sendNotification({ recipientId: 'emp6', entityId: 'ent1', type: 'leave_declined', vars: {} }, repo, ALL_PROVIDERS);
...
test('markRead: throws NOT_FOUND for unknown id', async () => {
    const repo = makeRepo();
    await assert.rejects(() => markRead('ntf_ghost', repo), (e) => e.code === 'NOT_FOUND');
});
