 ...
 */
import { renderTemplate, type NotifType, type Locale } from './templates.js';
import type { NotifRepo, SendInput, NotificationRecord, Channel, RecipientPreference } from './types.js';
...
const DEFAULT_PREF: Omit<RecipientPreference, 'recipientId'> = {
  locale: 'en',
  channels: ['in_app'],
};

function isQuietHour(pref: RecipientPreference): boolean {
  if (!pref.quietFrom || !pref.quietUntil) return false;
  const now = new Date();
...
export async function sendNotification(
  ...
  if (!input.recipientId) throw new NotifError('VALIDATION', 'recipientId is required', 'recipientId');
  if (!input.type)        throw new NotifError('VALIDATION', 'type is required', 'type');
  ...
  const { title, body } = renderTemplate(input.type as NotifType, input.vars, pref.locale as Locale);

  // 3. Determine channels — in_app always included; others from preference
  const channels: Channel[] = ['in_app', ...pref.channels.filter((c) => c !== 'in_app')];
  ...
  const effectiveChannels = channels.filter((c) => {
    ...
    if (quietActive && (priority === 'low' || priority === 'normal')) return false;
    return true;
  });
  ...
  const deliveries = await Promise.all(
    effectiveChannels.map((channel) =>
      providers[channel]?.send(
        { id: 'pending', recipientId: input.recipientId, entityId: input.entityId, type: input.type, priority, title, body, vars: input.vars, read: false, deliveries: [], createdAt: '' },
        contacts[channel],
      ).catch((err: unknown) => ({
        channel,
...
export async function markRead(
  ...
  if (!rec) throw new NotifError('NOT_FOUND', `Notification ${id} not found`);
