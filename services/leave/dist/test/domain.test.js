...
const ksaCal = { workWeek: [0, 1, 2, 3, 4], holidays: new Set() };
...
function makeRepo(overrides = {}) {
    ...
    return {
        ...
        updateStatus: async (id, status, expectedVersion, _event) => {
            ...
            if (!rec)
                throw new LeaveError('NOT_FOUND', `${id} not found`);
            if (rec.version !== expectedVersion)
                throw new LeaveError('CONFLICT', 'version mismatch');
...
test('createLeaveRequest happy path computes days + starts workflow', async () => {
    let savedEvent = null;
    const repo = makeRepo({
        saveWithEvent: async (r, e) => { repo._store = r; savedEvent = e; },
    });
    const rec = await createLeaveRequest({ entityId: 'ent1', employeeId: 'emp1', leaveTypeId: 'annual',
        startDate: '2026-03-15', endDate: '2026-03-22', idempotencyKey: 'k1' }, ksaCal, repo, wf, 'corr1');
...
test('createLeaveRequest is idempotent', async () => {
    const prior = { id: 'lv_existing', version: 1 };
...
test('createLeaveRequest rejects insufficient balance', async () => {
    ...
    await assert.rejects(() => createLeaveRequest({ entityId: 'ent1', employeeId: 'emp1', leaveTypeId: 'annual',
        startDate: '2026-03-15', endDate: '2026-03-22', idempotencyKey: 'k1' }, ksaCal, repo, wf, 'corr1'), (e) => e.code === 'INSUFFICIENT_BALANCE' && e.details?.available === 4);
});
test('createLeaveRequest rejects overlap', async () => {
    const repo = makeRepo({ hasOverlap: async () => true });
    await assert.rejects(() => createLeaveRequest({ entityId: 'ent1', employeeId: 'emp1', leaveTypeId: 'annual',
        startDate: '2026-03-15', endDate: '2026-03-22', idempotencyKey: 'k1' }, ksaCal, repo, wf, 'corr1'), (e) => e.code === 'OVERLAPPING_REQUEST');
});
test('createLeaveRequest rejects zero working days', async () => {
    /* 2026-03-20 = Fri, 2026-03-21 = Sat — no working days */
    await assert.rejects(() => createLeaveRequest({ entityId: 'ent1', employeeId: 'emp1', leaveTypeId: 'annual',
        startDate: '2026-03-20', endDate: '2026-03-21', idempotencyKey: 'k2' }, ksaCal, makeRepo(), wf, 'corr1'), (e) => e.code === 'INVALID_DATE_RANGE');
});
/* ─── cancelLeaveRequest ────────────────────────────────────── */
async function seedRequest(status = 'pending_approval') {
    const store = new Map();
    const id = newId('lv');
...
test('cancelLeaveRequest rejects wrong requester', async () => {
    ...
    await assert.rejects(() => cancelLeaveRequest(id, 'emp_other', 1, 'corr1', repo), (e) => e.code === 'FORBIDDEN');
});
test('cancelLeaveRequest rejects already-taken request', async () => {
    const { repo, id } = await seedRequest('taken');
    await assert.rejects(() => cancelLeaveRequest(id, 'emp1', 1, 'corr1', repo), (e) => e.code === 'INVALID_STATE_TRANSITION');
});
...
test('cancelLeaveRequest rejects stale version (ETag conflict)', async () => {
    const { repo, id } = await seedRequest();
    await assert.rejects(() => cancelLeaveRequest(id, 'emp1', 99, 'corr1', repo), (e) => e.code === 'CONFLICT');
});
test('cancelLeaveRequest rejects non-existent request', async () => {
    const repo = makeRepo();
    await assert.rejects(() => cancelLeaveRequest('lv_ghost', 'emp1', 1, 'corr1', repo), (e) => e.code === 'NOT_FOUND');
});
