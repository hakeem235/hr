...
class InMemoryLeaveRepo implements LeaveRepo {
  ...
  async findByIdempotencyKey(employeeId: string, key: string): Promise<LeaveRecord | null> {
    for (const r of this.store.values()) {
      if (r.employeeId === employeeId && r.idempotencyKey === key) return r;
    }
    return null;
  }
  ...
  async getBalance(_employeeId: string, leaveTypeId: string, _year: number): Promise<LeaveBalance> {
    /* Stub balances per type — replace with real DB query */
    const defaults: Record<string, LeaveBalance> = {
  ...
  async hasOverlap(employeeId: string, start: string, endDate: string, excludeId?: string): Promise<boolean> {
    for (const r of this.store.values()) {
      if (r.id === excludeId) continue;
      if (r.employeeId !== employeeId) continue;
      if (['cancelled', 'declined'].includes(r.status)) continue;
      /* Overlap: not (end < start || start > end) */
      if (!(endDate < r.startDate || start > r.endDate)) return true;
    }
    return false;
  }

  async saveWithEvent(rec: LeaveRecord, event: DomainEvent): Promise<void> {
  ...
  async updateStatus(
    ...
    if (!rec) throw new LeaveError('NOT_FOUND', `Leave request ${id} not found`);
  ...
  async listRequests(filter: ListFilter): Promise<{ items: LeaveRecord[]; nextCursor?: string }> {
    let all = [...this.store.values()];

    if (filter.employeeId) all = all.filter((r) => r.employeeId === filter.employeeId);
    if (filter.entityId)   all = all.filter((r) => r.entityId === filter.entityId);
...
async function calendarFor(_entityId: string): Promise<WorkingCalendar> {
