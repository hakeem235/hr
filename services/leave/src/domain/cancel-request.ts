...
import { newEvent, type LeaveRecord, type LeaveRepo, type DomainEvent } from './create-request.js';

const CANCELLABLE: LeaveRecord['status'][] = ['pending_approval', 'approved', 'scheduled'];
...
export async function cancelLeaveRequest(
  ...
  if (!CANCELLABLE.includes(rec.status)) {
    throw new LeaveError(
      ...
      `Cannot cancel a request with status '${rec.status}'`,
      undefined,
      { current: rec.status, allowed: CANCELLABLE },
    );
  }

  const event: DomainEvent = newEvent(
    'LeaveCancelled',
    rec.entityId,
    correlationId,
    rec.id,
