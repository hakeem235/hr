...
export function getLeaveTypes(entityId: string): LeaveType[] {
  return LEAVE_TYPES.filter((t) => t.entityId === entityId || t.entityId === DEFAULT_ENTITY);
}

export function getLeavePolicy(leaveTypeId: string, entityId: string): LeavePolicy | undefined {
  return LEAVE_POLICIES.find(
    (p) => p.leaveTypeId === leaveTypeId && (p.entityId === entityId || p.entityId === DEFAULT_ENTITY),
  );
}
