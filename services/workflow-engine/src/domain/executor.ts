...
import { computeSlaDueAt, KSA_CALENDAR } from './sla.js';
...
function makeEvent(
  ...
  return {
    eventId: randomUUID(),
    eventType: type,
    entityId: instance.context.entityId as string ?? 'unknown',
    correlationId: instance.context.correlationId as string ?? randomUUID(),
...
function getStep(def: WorkflowDefinition, stepId: string): StepDefinition {
  const step = def.steps.find((s) => s.id === stepId);
  if (!step) throw new EngineError('STEP_NOT_FOUND', `Step "${stepId}" not found in definition "${def.workflowId}" v${def.version}`);
  return step;
}

function nextStepId(stepDef: StepDefinition, event: string): string | null {
...
export class WorkflowExecutor {
  constructor(
    ...
    private calendarFor: (entityId: string) => Promise<WorkingCalendar>,
  ) {}
  ...
  async startInstance(
    ...
    if (!def) throw new EngineError('DEFINITION_NOT_FOUND', `No workflow definition found for "${workflowId}"`);
    if (def.deletedAt) throw new EngineError('DEFINITION_DELETED', `Workflow "${workflowId}" has been deleted`);
    ...
    if (!firstStep) throw new EngineError('EMPTY_DEFINITION', `Workflow "${workflowId}" has no steps`);
  ...
  private async activateApproval(
    instance: WorkflowInstance,
    def: WorkflowDefinition,
    stepDef: ApprovalStepDef,
  ): Promise<void> {
    const entityId = instance.context.entityId as string;
  ...
  private async executeAutomated(
    instance: WorkflowInstance,
    def: WorkflowDefinition,
    stepDef: import('./types.js').AutomatedStepDef,
  ): Promise<void> {
    const stepExec: StepExecution = {
      id: newId('step'),
      instanceId: instance.id,
  ...
  private async executeBranch(
    ...
    if (!nextId) nextId = stepDef.transitions[0]?.to ?? null;

    const done: StepExecution = { ...stepExec, state: 'done', decidedAt: new Date().toISOString() };
  ...
  private async activateWait(
    instance: WorkflowInstance,
    _def: WorkflowDefinition,
    stepDef: import('./types.js').WaitStepDef,
  ): Promise<void> {
    const slaDueAt = stepDef.until
      ? (resolvePath(stepDef.until, instance.context) as string ?? stepDef.until)
      : undefined;
  ...
  async processDecision(
    ...
    if (!instance) throw new EngineError('NOT_FOUND', `Workflow instance "${instanceId}" not found`);
    ...
    if (!def) throw new EngineError('DEFINITION_NOT_FOUND', `Definition not found for "${instance.workflowId}" v${instance.definitionVersion}`);
    ...
    if (!stepExec) throw new EngineError('STEP_NOT_ACTIVE', `No active execution found for step "${stepId}"`);
    ...
    const now = new Date().toISOString();
    const completedExec: StepExecution = {
      ...stepExec,
      state: 'done',
      decision,
      note,
  ...
  async escalateStep(instanceId: string, stepId: string): Promise<void> {
    const instance = await this.repo.getInstance(instanceId);
    ...
    const stepDef = getStep(def, stepId);
    if (stepDef.type !== 'approval') return;
    ...
    if (stepExec.escalationCount >= maxEscalations) {
      ...
      const failed: StepExecution = { ...stepExec, state: 'failed' };
  ...
  async cancelInstance(instanceId: string, reason: string): Promise<WorkflowInstance> {
    ...
    for (const exec of executions.filter((e) => e.state === 'active')) {
      const failed: StepExecution = { ...exec, state: 'failed', decidedAt: now };
  ...
  private async completeInstance(instance: WorkflowInstance, result: string): Promise<void> {
    const now = new Date().toISOString();
