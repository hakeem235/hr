...
export function parseDurationHours(duration) {
    // PT{n}H
    ...
    if (daysMatch)
        return parseInt(daysMatch[1]) * 8; // 8 working hours/day
    // P{n}W
    const weeksMatch = duration.match(/^P(\d+)W$/);
    if (weeksMatch)
        return parseInt(weeksMatch[1]) * 5 * 8; // 5 working days/week
 ...
 */
export function addBusinessHours(from, hours, cal) {
 ...
 */
export function computeSlaDueAt(from, duration, businessHours, cal) {
    if (!businessHours) {
...
export function isSlaBreached(slaDueAt) {
