...
export function registerDelegationRoutes(app, repo) {
    ...
    app.post('/api/v1/delegations', async (req, reply) => {
        try {
            ...
            const rec = await createDelegation({ fromEmployeeId: String(body.fromEmployeeId), toEmployeeId: String(body.toEmployeeId), validFrom: String(body.validFrom), validUntil: String(body.validUntil) }, repo);
    ...
    app.get('/api/v1/delegations', async (req, reply) => {
        const { fromEmployeeId } = req.query;
        if (!fromEmployeeId)
            return reply.status(422).send({ error: { code: 'VALIDATION', message: 'fromEmployeeId query param required' } });
