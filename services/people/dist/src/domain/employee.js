...
export async function createEmployee(input, repo, correlationId) {
...
export async function updateEmployeeStatus(id, newStatus, exitDate, expectedVersion, repo, correlationId) {
    const existing = await repo.findEmployeeById(id);
    if (!existing)
        throw new PeopleError('NOT_FOUND', `Employee ${id} not found`);
    ...
    if (!allowed.includes(newStatus)) {
        throw new PeopleError('VALIDATION', `Cannot transition from ${existing.status} to ${newStatus}`, 'status', { current: existing.status, requested: newStatus, allowed });
    }
    ...
    const eventType = newStatus === 'active' && existing.status === 'pre_hire' ? 'EmployeeOnboarded'
...
export async function listEmployees(filter, repo) {
