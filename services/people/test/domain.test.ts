...
import type {
  PeopleRepo, PersonRecord, EmployeeRecord, PositionRecord,
  CompensationRecord, DocumentRecord, DelegationRecord,
  EntityRecord, DepartmentRecord, HolidayRecord,
  DomainEvent, PersonFilter, EmployeeFilter, DocumentFilter,
  EmploymentStatus,
} from '../src/domain/types.js';
import { PeopleError } from '../src/domain/errors.js';
...
import { createPosition, getCurrentPosition } from '../src/domain/position.js';
import { createCompensation, getCurrentCompensation } from '../src/domain/compensation.js';
...
function makeRepo(overrides: Partial<PeopleRepo> = {}): PeopleRepo {
  const entities     = new Map<string, EntityRecord>();
  const departments  = new Map<string, DepartmentRecord>();
  const holidays     = new Map<string, HolidayRecord>();
  ...
  const base: PeopleRepo = {
    ...
    listPersons:       async (f: PersonFilter) => {
    ...
    listEmployees:     async (f: EmployeeFilter) => {
    ...
    listDocuments:     async (f: DocumentFilter) => {
    ...
    getActiveDelegation: async (fid, asOf) =>
      [...delegations.values()].find((d) => d.fromEmployeeId === fid && d.validFrom <= asOf && d.validUntil >= asOf) ?? null,
...
test('createPerson: creates record with correct fields', async () => {
  ...
  const rec = await createPerson(
    { fullNameEn: 'Ahmed Ali', nationality: 'SA', dateOfBirth: '1990-01-01', idempotencyKey: 'k1' },
    repo, 'corr1',
  );
...
test('createPerson: rejects bad dateOfBirth format', async () => {
  ...
  await assert.rejects(
    () => createPerson({ fullNameEn: 'X', nationality: 'SA', dateOfBirth: '01/01/1990', idempotencyKey: 'k2' }, repo, 'corr'),
    (e: PeopleError) => e.code === 'VALIDATION',
  );
});
...
test('updatePerson: updates fields and bumps version', async () => {
  ...
  const created = await createPerson(
    { fullNameEn: 'Ahmed Ali', nationality: 'SA', dateOfBirth: '1990-01-01', idempotencyKey: 'k3' },
    repo, 'corr',
  );
...
test('updatePerson: rejects stale version', async () => {
  const repo = makeRepo();
  const created = await createPerson(
    { fullNameEn: 'X', nationality: 'SA', dateOfBirth: '1990-01-01', idempotencyKey: 'k4' },
    repo, 'corr',
  );
  await assert.rejects(
    () => updatePerson(created.id, { fullNameEn: 'Y' }, 99, repo, 'corr'),
    (e: PeopleError) => e.code === 'CONFLICT',
  );
});
...
test('createEmployee: creates with pre_hire status', async () => {
  ...
  const rec = await createEmployee(
    { personId: 'per_001', entityId: 'ent_001', employeeNo: 'EMP-001', hireDate: '2026-01-01', idempotencyKey: 'emp_k1' },
    repo, 'corr',
  );
...
test('createEmployee: idempotent', async () => {
  ...
  const a = await createEmployee(
    { personId: 'per_001', entityId: 'ent_001', employeeNo: 'EMP-001', hireDate: '2026-01-01', idempotencyKey: 'emp_k2' },
    repo, 'corr',
  );
...
test('updateEmployeeStatus: pre_hire → active emits EmployeeOnboarded', async () => {
  ...
  const emp = await createEmployee(
    { personId: 'per_x', entityId: 'ent_x', employeeNo: 'E1', hireDate: '2026-01-01', idempotencyKey: 'esk1' },
    repo, 'corr',
  );
...
test('updateEmployeeStatus: requires exitDate when terminating', async () => {
  const repo = makeRepo();
  const emp = await createEmployee(
    { personId: 'per_x', entityId: 'ent_x', employeeNo: 'E2', hireDate: '2026-01-01', idempotencyKey: 'esk2' },
    repo, 'corr',
  );
  await assert.rejects(
    () => updateEmployeeStatus(emp.id, 'terminated', undefined, 1, repo, 'corr'),
    (e: PeopleError) => e.code === 'VALIDATION' && e.field === 'exitDate',
  );
});
...
test('updateEmployeeStatus: rejects invalid transition', async () => {
  const repo = makeRepo();
  const emp = await createEmployee(
    { personId: 'per_x', entityId: 'ent_x', employeeNo: 'E3', hireDate: '2026-01-01', idempotencyKey: 'esk3' },
    repo, 'corr',
  );
  await assert.rejects(
    () => updateEmployeeStatus(emp.id, 'on_leave', undefined, 1, repo, 'corr'),
    (e: PeopleError) => e.code === 'VALIDATION',
  );
});
...
test('createPosition: creates and closes previous', async () => {
  ...
  const pos1 = await createPosition(
    { employeeId: 'emp_1', title: 'Engineer', grade: 'L3', departmentId: 'dep_1', workflowRole: 'employee', effectiveFrom: '2024-01-01', idempotencyKey: 'pos_k1' },
    repo,
  );
  assert.equal(pos1.effectiveTo, undefined);

  const pos2 = await createPosition(
    { employeeId: 'emp_1', title: 'Senior Engineer', grade: 'L4', departmentId: 'dep_1', workflowRole: 'employee', effectiveFrom: '2025-01-01', idempotencyKey: 'pos_k2' },
    repo,
  );
...
test('createPosition: idempotent', async () => {
  ...
  const a = await createPosition(
    { employeeId: 'emp_2', title: 'Eng', grade: 'L1', departmentId: 'dep_1', workflowRole: 'employee', effectiveFrom: '2024-01-01', idempotencyKey: 'pos_k3' },
    repo,
  );
...
test('createCompensation: creates record in SAR halalas', async () => {
  ...
  const rec = await createCompensation(
    { employeeId: 'emp_3', basicMinor: 900000, effectiveFrom: '2026-01-01', idempotencyKey: 'cmp_k1' },
    repo,
  );
...
test('createCompensation: closes previous on new entry', async () => {
  ...
  await createCompensation(
    { employeeId: 'emp_3', basicMinor: 900000, effectiveFrom: '2024-01-01', idempotencyKey: 'cmp_k2' },
    repo,
  );
  await createCompensation(
    { employeeId: 'emp_3', basicMinor: 1000000, effectiveFrom: '2025-01-01', idempotencyKey: 'cmp_k3' },
    repo,
  );
...
test('createCompensation: rejects negative basicMinor', async () => {
  const repo = makeRepo();
  await assert.rejects(
    () => createCompensation({ employeeId: 'emp_x', basicMinor: -1, effectiveFrom: '2026-01-01', idempotencyKey: 'cmp_k4' }, repo),
    (e: PeopleError) => e.code === 'VALIDATION',
  );
});
...
test('createDelegation: creates valid delegation', async () => {
  const repo = makeRepo();
  // seed employees
  await repo.saveEmployee({ id: 'emp_a', personId: 'per_a', entityId: 'ent_1', employeeNo: 'A1', status: 'active', hireDate: '2024-01-01', idempotencyKey: 'ik_a', createdAt: '', version: 1 }, {} as DomainEvent);
  await repo.saveEmployee({ id: 'emp_b', personId: 'per_b', entityId: 'ent_1', employeeNo: 'B1', status: 'active', hireDate: '2024-01-01', idempotencyKey: 'ik_b', createdAt: '', version: 1 }, {} as DomainEvent);

  const dlg = await createDelegation(
    { fromEmployeeId: 'emp_a', toEmployeeId: 'emp_b', validFrom: '2026-05-01', validUntil: '2026-05-31' },
    repo,
  );
...
test('createDelegation: rejects self-delegation', async () => {
  const repo = makeRepo();
  await assert.rejects(
    () => createDelegation({ fromEmployeeId: 'emp_a', toEmployeeId: 'emp_a', validFrom: '2026-05-01', validUntil: '2026-05-31' }, repo),
    (e: PeopleError) => e.code === 'VALIDATION',
  );
});
...
test('createDelegation: rejects validUntil before validFrom', async () => {
  const repo = makeRepo();
  await assert.rejects(
    () => createDelegation({ fromEmployeeId: 'emp_a', toEmployeeId: 'emp_b', validFrom: '2026-06-01', validUntil: '2026-05-01' }, repo),
    (e: PeopleError) => e.code === 'INVALID_DATE_RANGE',
  );
});
...
test('createDelegation: rejects missing employee', async () => {
  const repo = makeRepo();
  await assert.rejects(
    () => createDelegation({ fromEmployeeId: 'emp_ghost', toEmployeeId: 'emp_b', validFrom: '2026-05-01', validUntil: '2026-05-31' }, repo),
    (e: PeopleError) => e.code === 'NOT_FOUND',
  );
});

test('deleteDelegation: removes delegation', async () => {
  const repo = makeRepo();
  await repo.saveEmployee({ id: 'emp_c', personId: 'per_c', entityId: 'ent_1', employeeNo: 'C1', status: 'active', hireDate: '2024-01-01', idempotencyKey: 'ik_c', createdAt: '', version: 1 }, {} as DomainEvent);
  await repo.saveEmployee({ id: 'emp_d', personId: 'per_d', entityId: 'ent_1', employeeNo: 'D1', status: 'active', hireDate: '2024-01-01', idempotencyKey: 'ik_d', createdAt: '', version: 1 }, {} as DomainEvent);
  const dlg = await createDelegation(
    { fromEmployeeId: 'emp_c', toEmployeeId: 'emp_d', validFrom: '2026-05-01', validUntil: '2026-05-31' },
    repo,
  );
...
async function seedEmpWithPos(repo: PeopleRepo, empId: string, managerId: string | undefined, role: PositionRecord['workflowRole']) {
  await repo.saveEmployee({ id: empId, personId: `per_${empId}`, entityId: 'ent_1', employeeNo: empId, status: 'active', hireDate: '2024-01-01', idempotencyKey: `ik_${empId}`, createdAt: '', version: 1 }, {} as DomainEvent);
