...
class InMemoryPeopleRepo implements PeopleRepo {
  private entities     = new Map<string, EntityRecord>();
  private holidays     = new Map<string, HolidayRecord>();  // key: entityId|date
  ...
  private compensations = new Map<string, CompensationRecord>();
  ...
  // ── Entity ──────────────────────────────────────────────────────────────────
  async findEntityById(id: string)       { return this.entities.get(id) ?? null; }
  async listEntities()                   { return [...this.entities.values()]; }
  async saveEntity(rec: EntityRecord, event: DomainEvent) {
  ...
  async upsertHoliday(rec: HolidayRecord) {
    this.holidays.set(`${rec.entityId}|${rec.holidayDate}`, rec);
  }
  async deleteHoliday(entityId: string, holidayDate: string) {
    this.holidays.delete(`${entityId}|${holidayDate}`);
  }

  // ── Department ──────────────────────────────────────────────────────────────
  async findDepartmentById(id: string)          { return this.departments.get(id) ?? null; }
  async listDepartments(entityId: string)       { return [...this.departments.values()].filter((d) => d.entityId === entityId); }
  async saveDepartment(rec: DepartmentRecord, event: DomainEvent) {
    this.departments.set(rec.id, rec);
    this.log(event);
  }

  // ── Person ───────────────────────────────────────────────────────────────────
  async findPersonById(id: string)              { return this.persons.get(id) ?? null; }
  async findPersonByIdempotencyKey(key: string) { return [...this.persons.values()].find((p) => p.idempotencyKey === key) ?? null; }
  async listPersons(filter: PersonFilter) {
  ...
  async savePerson(rec: PersonRecord, event: DomainEvent) {
    this.persons.set(rec.id, rec);
    this.log(event);
  }

  // ── Employee ─────────────────────────────────────────────────────────────────
  async findEmployeeById(id: string)              { return this.employees.get(id) ?? null; }
  async findEmployeeByIdempotencyKey(key: string) { return [...this.employees.values()].find((e) => e.idempotencyKey === key) ?? null; }
  async listEmployees(filter: EmployeeFilter) {
    ...
    if (filter.role) {
      const withRole: EmployeeRecord[] = [];
      for (const emp of all) {
        const pos = await this.getCurrentPosition(emp.id);
        if (pos?.workflowRole === filter.role) withRole.push(emp);
      }
      all = withRole;
    }
  ...
  async saveEmployee(rec: EmployeeRecord, event: DomainEvent) {
    this.employees.set(rec.id, rec);
    this.log(event);
  }
  async updateEmployeeStatus(id: string, status: EmploymentStatus, exitDate: string | undefined, _expectedVersion: number, event: DomainEvent) {
    const rec = this.employees.get(id)!;
    const updated: EmployeeRecord = { ...rec, status, exitDate, version: rec.version + 1 };
  ...
  // ── Position ─────────────────────────────────────────────────────────────────
  async findPositionByIdempotencyKey(key: string) { return [...this.positions.values()].find((p) => p.idempotencyKey === key) ?? null; }
  async listPositions(employeeId: string)         { return [...this.positions.values()].filter((p) => p.employeeId === employeeId).sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom)); }
  async getCurrentPosition(employeeId: string, asOf?: string) {
  ...
  async savePosition(rec: PositionRecord) { this.positions.set(rec.id, rec); }

  // ── Compensation ─────────────────────────────────────────────────────────────
  async findCompensationByIdempotencyKey(key: string) { return [...this.compensations.values()].find((c) => c.idempotencyKey === key) ?? null; }
  async listCompensation(employeeId: string)          { return [...this.compensations.values()].filter((c) => c.employeeId === employeeId).sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom)); }
  async getCurrentCompensation(employeeId: string, asOf?: string) {
    const date = asOf ?? new Date().toISOString().slice(0, 10);
  ...
  async saveCompensation(rec: CompensationRecord) { this.compensations.set(rec.id, rec); }

  // ── Document ─────────────────────────────────────────────────────────────────
  async findDocumentById(id: string)              { return this.documents.get(id) ?? null; }
  async findDocumentByIdempotencyKey(key: string) { return [...this.documents.values()].find((d) => d.idempotencyKey === key) ?? null; }
  ...
  async saveDocument(rec: DocumentRecord, event: DomainEvent) {
    this.documents.set(rec.id, rec);
    this.log(event);
  }

  // ── Delegation ───────────────────────────────────────────────────────────────
  async findDelegationById(id: string)             { return this.delegations.get(id) ?? null; }
  async listDelegations(fromEmployeeId: string)    { return [...this.delegations.values()].filter((d) => d.fromEmployeeId === fromEmployeeId); }
  async getActiveDelegation(fromEmployeeId: string, asOf: string) {
    return [...this.delegations.values()].find(
      (d) => d.fromEmployeeId === fromEmployeeId && d.validFrom <= asOf && d.validUntil >= asOf,
    ) ?? null;
  }
  async saveDelegation(rec: DelegationRecord)      { this.delegations.set(rec.id, rec); }
  async deleteDelegation(id: string)               { this.delegations.delete(id); }
}
...
const seedPersons: PersonRecord[] = [
...
// ─── Employees (IDs match workflow engine + leave service) ────────────────────
type SeedEmployee = { id: string; personId: string; employeeNo: string; status: EmploymentStatus };
...
// ─── Positions ────────────────────────────────────────────────────────────────
type SeedPosition = { id: string; employeeId: string; title: string; grade: string; departmentId: string; reportsTo?: string; workflowRole: PositionRecord['workflowRole'] };
