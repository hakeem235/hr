...
export async function createDelegation(
  ...
  if (!from) throw new PeopleError('NOT_FOUND', `Employee ${input.fromEmployeeId} not found`, 'fromEmployeeId');

  const to = await repo.findEmployeeById(input.toEmployeeId);
  if (!to) throw new PeopleError('NOT_FOUND', `Employee ${input.toEmployeeId} not found`, 'toEmployeeId');
...
export async function deleteDelegation(
  ...
  if (!existing) throw new PeopleError('NOT_FOUND', `Delegation ${id} not found`);
 ...
 */
export async function buildOrgNode(
  employeeId: string,
  repo: PeopleRepo,
  asOf?: string,
