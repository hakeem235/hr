...
export function registerPayrollRoutes(app: FastifyInstance, repo: PayrollRepo): void {
  ...
  app.post('/api/v1/payroll-runs', async (req, reply) => {
    ...
    try {
      ...
      const run = await createPayrollRun(
        { entityId: String(body.entityId), period: String(body.period), idempotencyKey },
        repo, correlationId,
      );
  ...
  app.get('/api/v1/payroll-runs', async (req, reply) => {
    const q = req.query as { entityId?: string; period?: string; status?: string; cursor?: string; limit?: string };
    const limit = Math.min(Number(q.limit ?? 20), 100);
    const { items, nextCursor } = await repo.listRuns({
      entityId: q.entityId,
      period: q.period,
      status: q.status as PayrollRunStatus | undefined,
      cursor: q.cursor,
      limit,
    });
  ...
  app.post('/api/v1/payroll-runs/:id/calculate', async (req, reply) => {
    ...
    try {
      const body = req.body as { employees?: unknown[] };
      ...
      const { run, payslips } = await calculatePayrollRun(
        id, body.employees as Parameters<typeof calculatePayrollRun>[1],
        ifMatch, repo, correlationId,
      );
