...
import type {
  PayrollRun, PayrollRunStatus, PayslipRecord, EmployeePayInput, PayrollRepo,
} from './types.js';
...
const TRANSITIONS: Partial<Record<PayrollRunStatus, PayrollRunStatus[]>> = {
...
const CANCELLABLE: PayrollRunStatus[] = ['draft', 'calculating', 'pending_approval', 'approved'];

function assertTransition(run: PayrollRun, to: PayrollRunStatus): void {
  const allowed = TRANSITIONS[run.status] ?? [];
  if (!allowed.includes(to)) {
    throw new PayrollError(
      'INVALID_STATE',
      `Cannot transition run from '${run.status}' to '${to}'`,
    );
  }
}

function assertVersion(run: PayrollRun, expected: number): void {
...
export async function calculatePayrollRun(
  ...
  if (!run) throw new PayrollError('NOT_FOUND', `PayrollRun ${runId} not found`);
  ...
  for (const emp of employees) {
    ...
    if (net < 0) {
      throw new PayrollError(
        'VALIDATION',
        `Net pay is negative for employee ${emp.employeeId} — deductions exceed gross`,
