...
import {
  createPayrollRun, calculatePayrollRun, approvePayrollRun,
  submitWps, markRunPaid, cancelPayrollRun,
} from '../src/domain/payroll-run.js';
import { PayrollError } from '../src/domain/errors.js';
import type { PayrollRepo, PayrollRun, PayslipRecord, DomainEvent, RunFilter, PayslipStatus } from '../src/domain/types.js';

// ── Minimal in-memory repo for tests ─────────────────────────────────────────

function makeRepo(): PayrollRepo {
  const runs     = new Map<string, PayrollRun>();
  const byKey    = new Map<string, PayrollRun>();
  const payslips = new Map<string, PayslipRecord>();
  const outbox   = new Map<string, DomainEvent>();

  return {
    async findRunByIdempotencyKey(key) { return byKey.get(key) ?? null; },
    async saveRun(run, event) {
      runs.set(run.id, run); byKey.set(run.idempotencyKey, run); outbox.set(event.eventId, event);
      return run;
    },
    ...
    async savePayslips(slips) { for (const s of slips) payslips.set(s.id, s); },
    ...
    async updatePayslipStatus(runId, status: PayslipStatus) {
      for (const [id, slip] of payslips) {
        if (slip.payrollRunId === runId) payslips.set(id, { ...slip, status });
      }
    },
...
describe('GOSI: integer safety', () => {
  ...
  it('rejects negative basicMinor', () => {
    assert.throws(
      () => calculateGosi('emp_01', -1, 'SA'),
      (e: unknown) => e instanceof PayrollError && e.code === 'VALIDATION',
    );
  });
});
...
describe('createPayrollRun', () => {
  ...
  it('rejects invalid period format', async () => {
    ...
    await assert.rejects(
      () => createPayrollRun({ entityId: ENTITY, period: '06-2026', idempotencyKey: 'key_03' }, repo, CORR),
      (e: unknown) => e instanceof PayrollError && e.code === 'VALIDATION',
    );
  });
});
...
describe('calculatePayrollRun', () => {
  ...
  it('rejects non-draft run', async () => {
    ...
    await assert.rejects(
      () => calculatePayrollRun(approved.id, EMPLOYEES, approved.version, repo, CORR),
      (e: unknown) => e instanceof PayrollError && e.code === 'INVALID_STATE',
    );
  });
  ...
  it('rejects stale ETag', async () => {
    const { repo, run } = await withDraft();
    await assert.rejects(
      () => calculatePayrollRun(run.id, EMPLOYEES, 99, repo, CORR),
      (e: unknown) => e instanceof PayrollError && e.code === 'CONFLICT',
    );
  });
  ...
  it('rejects empty employee list', async () => {
    const { repo, run } = await withDraft();
    await assert.rejects(
      () => calculatePayrollRun(run.id, [], run.version, repo, CORR),
      (e: unknown) => e instanceof PayrollError && e.code === 'VALIDATION',
    );
  });
  ...
  it('rejects when net pay would be negative', async () => {
    ...
    await assert.rejects(
      () => calculatePayrollRun(run.id, badEmployee, run.version, repo, CORR),
      (e: unknown) => e instanceof PayrollError && e.code === 'VALIDATION',
    );
  });
});
...
describe('approvePayrollRun', () => {
  ...
  it('rejects stale ETag', async () => {
    const { repo, run } = await withPendingApproval();
    await assert.rejects(
      () => approvePayrollRun(run.id, 99, repo, CORR),
      (e: unknown) => e instanceof PayrollError && e.code === 'CONFLICT',
    );
  });
});
...
describe('cancelPayrollRun', () => {
  ...
  it('rejects cancel from paid', async () => {
    ...
    const paid = await markRunPaid(wps.id, wps.version, repo, CORR);
    await assert.rejects(
      () => cancelPayrollRun(paid.id, paid.version, repo, CORR),
      (e: unknown) => e instanceof PayrollError && e.code === 'INVALID_STATE',
    );
  });
});
