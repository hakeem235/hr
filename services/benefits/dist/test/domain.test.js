...
/* ─── Fake repo ───────────────────────────────────────────────── */
function makeRepo(planOverrides = []) {
    const plans = new Map();
    const enrollments = new Map();
    // Load default + override plans
    for (const p of planOverrides)
        plans.set(p.id, p);
    return {
        ...
        findActiveEnrollment: async (empId, planId) => [...enrollments.values()].find((e) => e.employeeId === empId && e.planId === planId && (e.status === 'active' || e.status === 'pending')) ?? null,
        listEnrollments: async (f) => ({ items: [...enrollments.values()].slice(0, f.limit) }),
...
function repoWithPlan(plan = DEFAULT_PLANS[0]) {
...
test('EOSB: rejects exitDate before hireDate', () => {
    assert.throws(() => calculateEosb('emp1', '2026-01-01', '2025-01-01', 900000, 'voluntary'), (e) => e.code === 'VALIDATION');
});
test('EOSB: rejects negative salary', () => {
    assert.throws(() => calculateEosb('emp1', '2020-01-01', '2026-01-01', -1, 'voluntary'), (e) => e.code === 'VALIDATION');
});
/* ─── Default plans ───────────────────────────────────────────── */
...
test('createEnrollment: happy path creates pending enrollment', async () => {
    ...
    const rec = await createEnrollment({ entityId: 'ent1', employeeId: 'emp1', planId: DEFAULT_PLANS[0].id, effectiveFrom: '2026-01-01', idempotencyKey: 'k1' }, repo, 'corr');
...
test('createEnrollment: rejects missing plan', async () => {
    const repo = makeRepo([]);
    await assert.rejects(() => createEnrollment({ entityId: 'ent1', employeeId: 'emp1', planId: 'plan_ghost', effectiveFrom: '2026-01-01', idempotencyKey: 'k3' }, repo, 'corr'), (e) => e.code === 'NOT_FOUND');
});
test('createEnrollment: rejects inactive plan', async () => {
    ...
    await assert.rejects(() => createEnrollment({ entityId: 'ent1', employeeId: 'emp1', planId: 'plan_inactive', effectiveFrom: '2026-01-01', idempotencyKey: 'k4' }, repo, 'corr'), (e) => e.code === 'INELIGIBLE');
});
test('createEnrollment: rejects duplicate active enrollment', async () => {
    ...
    await assert.rejects(() => createEnrollment({ entityId: 'ent1', employeeId: 'emp1', planId: DEFAULT_PLANS[0].id, effectiveFrom: '2026-02-01', idempotencyKey: 'k6' }, repo, 'corr'), (e) => e.code === 'ALREADY_ENROLLED');
});
/* ─── activateEnrollment ──────────────────────────────────────── */
...
test('activateEnrollment: rejects non-pending status', async () => {
    ...
    await assert.rejects(() => activateEnrollment(enr.id, 2, repo, 'corr'), (e) => e.code === 'INVALID_STATE');
});
/* ─── cancelEnrollment ────────────────────────────────────────── */
...
test('cancelEnrollment: rejects stale ETag', async () => {
    ...
    await assert.rejects(() => cancelEnrollment(enr.id, '2026-12-31', 99, repo, 'corr'), (e) => e.code === 'CONFLICT');
});
/* ─── Dependents ──────────────────────────────────────────────── */
...
test('addDependent: rejects plan that does not allow dependents', async () => {
    ...
    await assert.rejects(() => addDependent(enr.id, { nameEn: 'X', relationship: 'spouse', dateOfBirth: '1990-01-01' }, repo), (e) => e.code === 'INELIGIBLE');
});
test('addDependent: rejects when max dependents reached', async () => {
    const plan = { ...DEFAULT_PLANS[0], id: 'plan_max1', maxDependents: 1 };
    ...
    await assert.rejects(() => addDependent(enr.id, { nameEn: 'Dep2', relationship: 'child', dateOfBirth: '2015-01-01' }, repo), (e) => e.code === 'INELIGIBLE' && e.details?.max === 1);
});
...
test('removeDependent: rejects unknown dependent id', async () => {
    ...
    await assert.rejects(() => removeDependent(enr.id, 'dep_ghost', repo), (e) => e.code === 'NOT_FOUND');
});
