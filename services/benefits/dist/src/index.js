...
class InMemoryBenefitRepo {
    ...
    async findPlanById(id) { return this.plans.get(id) ?? null; }
    async listPlans(entityId) {
        return [...this.plans.values()].filter((p) => p.entityId === entityId || p.entityId === 'ent_default');
    }
    async savePlan(plan, event) {
        this.plans.set(plan.id, plan);
        this.log(event);
    }
    async findEnrollmentById(id) { return this.enrollments.get(id) ?? null; }
    async findEnrollmentByIdempotencyKey(key) { return [...this.enrollments.values()].find((e) => e.idempotencyKey === key) ?? null; }
    async findActiveEnrollment(employeeId, planId) {
        return [...this.enrollments.values()].find((e) => e.employeeId === employeeId && e.planId === planId && (e.status === 'active' || e.status === 'pending')) ?? null;
    }
    ...
    async updateStatus(id, status, effectiveTo, _expectedVersion, event) {
    ...
    async addDependent(enrollmentId, dependent) {
    ...
    async removeDependent(enrollmentId, dependentId) {
        const rec = this.enrollments.get(enrollmentId);
        const updated = {
            ...rec,
