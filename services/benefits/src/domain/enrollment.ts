import type { EnrollmentRecord, EnrollmentStatus, BenefitRepo, Dependent } from './types.js';
...
const CANCELLABLE: EnrollmentStatus[] = ['pending', 'active', 'suspended'];
...
export async function createEnrollment(
  ...
  if (!plan) throw new BenefitError('NOT_FOUND', `Benefit plan ${input.planId} not found`, 'planId');
  if (!plan.isActive) throw new BenefitError('INELIGIBLE', `Benefit plan ${input.planId} is not active`);
  ...
  await repo.saveWithEvent(
    rec,
    newEvent('EmployeeEnrolled', input.entityId, correlationId, rec.id, {
      enrollmentId: rec.id,
...
export async function activateEnrollment(
  ...
  if (!rec) throw new BenefitError('NOT_FOUND', `Enrollment ${id} not found`);
  if (rec.status !== 'pending') throw new BenefitError('INVALID_STATE', `Cannot activate from status '${rec.status}'`);
  if (rec.version !== expectedVersion) throw new BenefitError('CONFLICT', 'Version mismatch', undefined, { expected: expectedVersion, current: rec.version });

  return repo.updateStatus(id, 'active', undefined, expectedVersion,
    newEvent('EnrollmentActivated', rec.entityId, correlationId, id, {
...
export async function cancelEnrollment(
  ...
  return repo.updateStatus(id, 'terminated', effectiveTo, expectedVersion,
    newEvent('EnrollmentCancelled', rec.entityId, correlationId, id, {
      enrollmentId: id,
      employeeId: rec.employeeId,
      planId: rec.planId,
      effectiveTo,
...
export async function addDependent(
  enrollmentId: string,
  input: Omit<Dependent, 'id' | 'enrollmentId' | 'addedAt'>,
  repo: BenefitRepo,
): Promise<EnrollmentRecord> {
  const rec = await repo.findEnrollmentById(enrollmentId);
  if (!rec) throw new BenefitError('NOT_FOUND', `Enrollment ${enrollmentId} not found`);
  ...
  if (!plan?.allowsDependents) throw new BenefitError('INELIGIBLE', 'This benefit plan does not allow dependents');
...
export async function removeDependent(
  ...
  if (!exists) throw new BenefitError('NOT_FOUND', `Dependent ${dependentId} not found in enrollment ${enrollmentId}`);
