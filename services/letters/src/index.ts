...
class InMemoryLetterRepo implements LetterRepo {
  private store  = new Map<string, LetterRecord>();
  private outbox: DomainEvent[] = [];

  async findByIdempotencyKey(employeeId: string, key: string): Promise<LetterRecord | null> {
    return [...this.store.values()].find(
      (r) => r.employeeId === employeeId && r.idempotencyKey === key,
    ) ?? null;
  }

  async findById(id: string): Promise<LetterRecord | null> {
    return this.store.get(id) ?? null;
  }

  async saveWithEvent(rec: LetterRecord, event: DomainEvent): Promise<void> {
  ...
  async updateStatus(
    ...
    if (rec.version !== expectedVersion) throw new Error('Version mismatch');
  ...
  async listRequests(filter: ListFilter): Promise<{ items: LetterRecord[]; nextCursor?: string }> {
...
const seedEvent = (type: string, id: string): DomainEvent => ({
...
const seedRecords: LetterRecord[] = [
  {
