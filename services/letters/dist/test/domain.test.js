...
import { createLetterRequest, cancelLetterRequest, markLetterIssued, } from '../src/domain/letter.js';
...
function makeRepo(overrides = {}) {
    const store = new Map();
    return {
        findByIdempotencyKey: async (employeeId, key) => [...store.values()].find((r) => r.employeeId === employeeId && r.idempotencyKey === key) ?? null,
        findById: async (id) => store.get(id) ?? null,
        saveWithEvent: async (rec) => { store.set(rec.id, rec); },
        updateStatus: async (id, status, expectedVersion, _event, extra) => {
            const rec = store.get(id);
            if (!rec)
                throw new LetterError('NOT_FOUND', `${id} not found`);
            if (rec.version !== expectedVersion)
                throw new LetterError('CONFLICT', 'version mismatch');
            const updated = { ...rec, status, version: rec.version + 1, updatedAt: new Date().toISOString(), ...extra };
            store.set(id, updated);
            return updated;
        },
        listRequests: async (_f) => ({ items: [], nextCursor: undefined }),
        ...overrides,
    };
}
...
test('createLetterRequest happy path starts workflow + saves record', async () => {
    let savedEvent = null;
    const repo = makeRepo({
        saveWithEvent: async (r, e) => { repo._last = r; savedEvent = e; },
    });
    const rec = await createLetterRequest({ entityId: 'ent1', employeeId: 'emp1', letterTypeId: 'salary_certificate',
        purpose: 'Bank account opening', language: 'bilingual', idempotencyKey: 'k1' }, repo, wf, 'corr1');
...
test('createLetterRequest is idempotent', async () => {
    const prior = { id: 'ltr_existing', version: 1 };
...
test('createLetterRequest rejects empty purpose', async () => {
    const repo = makeRepo();
    await assert.rejects(() => createLetterRequest({ entityId: 'ent1', employeeId: 'emp1', letterTypeId: 'noc',
        purpose: '   ', language: 'en', idempotencyKey: 'k2' }, repo, wf, 'corr'), (e) => e.code === 'POLICY_VIOLATION' && e.field === 'purpose');
});
test('createLetterRequest wraps workflow failure as WORKFLOW_UNAVAILABLE', async () => {
    ...
    await assert.rejects(() => createLetterRequest({ entityId: 'ent1', employeeId: 'emp1', letterTypeId: 'noc',
        purpose: 'visa', language: 'en', idempotencyKey: 'k3' }, repo, failWf, 'corr'), (e) => e.code === 'WORKFLOW_UNAVAILABLE');
});
/* ─── cancelLetterRequest ─────────────────────────────────────── */
...
async function repoWithRecord(rec) {
    const repo = makeRepo();
...
test('cancelLetterRequest rejects wrong requester', async () => {
    const repo = await repoWithRecord(seedRecord());
    await assert.rejects(() => cancelLetterRequest('ltr_001', 'emp_other', 1, repo, 'corr'), (e) => e.code === 'FORBIDDEN');
});
test('cancelLetterRequest rejects issued status', async () => {
    const repo = await repoWithRecord(seedRecord({ status: 'issued', version: 3 }));
    await assert.rejects(() => cancelLetterRequest('ltr_001', 'emp1', 3, repo, 'corr'), (e) => e.code === 'INVALID_STATE');
});
test('cancelLetterRequest rejects declined status', async () => {
    const repo = await repoWithRecord(seedRecord({ status: 'declined' }));
    await assert.rejects(() => cancelLetterRequest('ltr_001', 'emp1', 1, repo, 'corr'), (e) => e.code === 'INVALID_STATE');
});
test('cancelLetterRequest rejects stale ETag', async () => {
    const repo = await repoWithRecord(seedRecord({ version: 3 }));
    await assert.rejects(() => cancelLetterRequest('ltr_001', 'emp1', 1, repo, 'corr'), (e) => e.code === 'CONFLICT');
});
test('cancelLetterRequest rejects non-existent request', async () => {
    const repo = makeRepo();
    await assert.rejects(() => cancelLetterRequest('ltr_ghost', 'emp1', 1, repo, 'corr'), (e) => e.code === 'NOT_FOUND');
});
/* ─── markLetterIssued ────────────────────────────────────────── */
...
test('markLetterIssued rejects wrong initial status', async () => {
    ...
    await assert.rejects(() => markLetterIssued('ltr_001', 'doc_x', 1, repo, 'corr'), (e) => e.code === 'INVALID_STATE');
});
